...
### Enforcing Standards with `.clinerules`
If your team has strict coding standards (e.g., "Always use 3rd person in Gherkin", or "Page Objects must expose public fields"), you can enforce them automatically.

1.  **Define Rules**: Create a `.clinerules` file in your root.
    ```markdown
    # BDD Standards
    1. Feature files must use imperative style ("I login...", not "User logins...").
    2. Page Objects must NEVER expose `page` or `locator` directly; use action methods only.
    3. Step Definitions must be strictly typed.
    ```
2.  **The Integration**: When you prompt the AI, it reads these rules *first*.
3.  **Result**: The code generated by the MCP workflow will inherently obey your specific architectural constraints without you needing to correct it manually.